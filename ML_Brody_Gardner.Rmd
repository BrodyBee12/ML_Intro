---
title: "2_Teaching_Brody_Gardner"
author: "Brody G"
date: "2024-12-03"
...
#New model Training 
xgb_mod_2 <- xgboost(data = train_matrix, 
                   nrounds = 5000,
                   objective = "reg:squarederror", 
                   print_every_n = 1000,
...
test_sdd$sdd_pred_xgb_2 <- predict(xgb_mod_2, test_matrix)
...
# Visualization and evaluation on New Model 

ggplot(test_sdd, aes(y = sdd_pred_xgb_2, x = harmonized_value)) + 
  geom_point() + 
  xlab('Observed') + 
  ylab('Predicted') + 
  geom_smooth(method = 'lm', se = F) + 
...
#Test on New Model 
mape(test_sdd$harmonized_value, test_sdd$sdd_pred_xgb_2)
rmse(test_sdd$harmonized_value, test_sdd$sdd_pred_xgb_2)
...
#Barely any, I missed with the parameters for a few hours and I think there is somehting I am missing because the only thing that seems to change outside of some outliters (by altering how many simulations you run or altering early stopping rounds, etc.), the objetive is really all the matters which does make sense, since if the data is already precise, no matter how many times you run its liekly not the change, unless you are looking to predict a different varible or result.

What would be a systematic way to 

#I don't know how to answer this without the rest of the question and I apologize if it was corrected in class today, I missed it :/ 
